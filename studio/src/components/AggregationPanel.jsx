...
const AggregationPanel = ({ selectedNode, nodes, links, onClose }) => {
  ...
  const performAggregation = async () => {
    ...
    try {
      ...
      // Perform aggregation based on type and groupBy
      const aggregated = performCalculation(allData, aggregationType, groupBy);
  ...
  const performCalculation = (data, aggType, groupByField) => {
    const groups = {};
    
    // Group data by the specified field
    data.forEach(row => {
      const groupKey = row[0]; // Assuming first column is year
      if (!groups[groupKey]) {
        groups[groupKey] = [];
      }
    ...
    Object.entries(groups).forEach(([group, rows]) => {
      ...
      // Find numeric columns (skip first column which is usually the group field)
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        <div>
          <h3 className="text-md font-medium mb-3">Aggregation Settings</h3>
          <div className="space-y-4">
            <div>
              <label className="block text-sm text-gray-300 mb-2">Aggregation Type</label>
              <select
            ...
            <div>
              <label className="block text-sm text-gray-300 mb-2">Group By</label>
              <select
                value={groupBy}
